  ...
  extends ListenerAdapter
{
  public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
    ...
    if (args[0].equalsIgnoreCase("^selfrole")) {
      ...
      if (args.length == 1) {
        ...
        returnmsg = String.valueOf(returnmsg) + "\n\n**Use ^selfrole {role} to add/remove a role.**";
        returnmsg = String.valueOf(returnmsg) + "\n**Staff can use ^addselfrole {role id} and ^removeselfrole {role id} to make a role a selfrole or stop it being one.**";
  ...
  public int getMemberCount(Role r) { return r.getGuild().getMembersWithRoles(new Role[] { r }).size(); }
}
